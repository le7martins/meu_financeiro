      ...
      <div style={{...S.modal,maxHeight:"auto"}} className="modal-in" role="dialog" aria-modal="true" aria-label="Pagar Fatura">
        ...
        <div style={S.mHeader}>
          <div><div style={S.mTitle}>Pagar Fatura</div>
            <div style={{fontSize:11,color:card.color,marginTop:2,fontWeight:600,display:"flex",alignItems:"center",gap:4}}><CreditCard size={12} strokeWidth={2}/>{card.name}</div>
          </div>
          <button style={S.xBtn} onClick={onClose} aria-label="Fechar">✕</button>
        </div>
        ...
        {val>0&&val<maxToPay&&(
          <div style={{marginBottom:14,background:"rgba(250,204,21,.08)",border:"1px solid #facc1533",borderRadius:10,padding:"10px 14px"}}>
            <div style={{fontSize:11,color:"#facc15",fontWeight:600,marginBottom:3,display:"flex",alignItems:"center",gap:4}}><AlertTriangle size={11} strokeWidth={2.5}/>Pagamento parcial</div>
        ...
        {val>=maxToPay&&(
          <div style={{marginBottom:14,background:"rgba(74,222,128,.08)",border:"1px solid #4ade8033",borderRadius:10,padding:"10px 14px"}}>
            <div style={{fontSize:12,color:"#4ade80",fontWeight:600,display:"flex",alignItems:"center",gap:4}}><Check size={13} strokeWidth={2.5}/>Pagamento integral</div>
          </div>
        )}
