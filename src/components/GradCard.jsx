export default function GradCard({label,value,color,bg,icon,onAdd,empty}){
  return(<div className="gradCard" style={{background:bg,border:`1px solid ${empty?"var(--border)":color+"22"}`,borderRadius:16,padding:"14px 14px",position:"relative",overflow:"hidden",opacity:empty?0.6:1,transition:"opacity .2s"}}>
    ...
    <div className="gradCardValue" style={{fontSize:empty?22:18,fontWeight:800,color:empty?"var(--text4)":"var(--text1)",letterSpacing:"-0.5px"}}>{empty?"—":value}</div>
  </div>);
}
